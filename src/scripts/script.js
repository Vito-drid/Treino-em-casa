...
document.addEventListener('keydown', event => {
    if (event.code === 'Space') {
        ...
        if (currentTaskIndex < checkboxes.length) {
            ...
            if (currentTaskIndex + 1 < checkboxes.length) {
                checkboxes[currentTaskIndex + 1].disabled = false; // Habilitar a próxima checkbox
            }
...
// Função para alternar o slider (menu lateral)
function toggleSlider() {
    const slider = document.querySelector('.slider');
    const burgerMenu = document.querySelector('.burguer-menu');

    slider.classList.toggle('show');
    burgerMenu.style.left = slider.classList.contains('show') ? '220px' : '0';
}
